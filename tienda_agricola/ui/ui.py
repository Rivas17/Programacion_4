...
class MenuPrincipal:
    ...
    def crear_factura(self):
        ...
        while True:
            ...
            for contador,producto in enumerate(self.productos):
                print(f"{contador+1}. {producto.get_nombre()}")
            ...
            print(f"Total actual: {factura.get_valor_total()}")
            
        if factura.get_valor_total() > 0:
    ...
    def mostrar_cliente(self):
        ...
        for cliente in self.clientes:
            print(f"Nombre: {cliente.get_nombre()} - Cedula: {cliente.get_cedula()}")
            print(f"Facturas: {len(cliente.historial_pedidos())}\n")
    
    def mostrar_facturas(self):
        print("\nFACTURAS")
        for factura in self.facturass:
            print(f"Cliente: {factura.get_cliente().get_nombre()}")
            print(f"Fecha: {factura.get_mostrar_fecha()}")
            print(f"Total: ${factura.get_valor_total()}")
